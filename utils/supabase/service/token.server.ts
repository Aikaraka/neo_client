...
export async function getUserToken() {
  ...
  if (tokenError) throw new Error("조각을 가져올 수 없습니다.");
...
export async function claimDailyFreeTokens() {
  ...
  // 먼저 사용자의 현재 조각 정보와 마지막 무료 조각 수령일 확인
  ...
  if (tokenError && tokenError.code === "PGRST116") {
    ...
    if (insertError) throw new Error("조각 생성에 실패했습니다.");
    ...
    return { success: true, message: "무료 조각 5개가 지급되었습니다!", newTokens: 5 };
  }

  if (tokenError) throw new Error("조각 정보를 조회할 수 없습니다.");
  ...
  if (tokenData.last_reset_date) {
    ...
    // 이미 오늘 무료 조각을 받았는지 확인
    if (lastResetDateString === todayDateString) {
      return { success: false, message: "오늘은 이미 무료 조각을 받으셨습니다. 내일 다시 시도해주세요!" };
    }
  }
  ...
  if (updateError) throw new Error("무료 조각 지급에 실패했습니다.");
  ...
  return { success: true, message: "무료 조각 5개가 지급되었습니다!", newTokens: newTokenBalance };
}
