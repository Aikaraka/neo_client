    ...
    const fetchPeriods = async () => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          // 중복 제거
          const periodLabels = data.map((item: PeriodLabel) => item.period_label);
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">랭킹 타입</label>
          <select
            value={rankingType}
      ...
      {isLoading ? (
        ...
        <div className="bg-yellow-100 p-4 rounded">
          <p className="text-yellow-700">
            {selectedPeriod 
              ? `${selectedPeriod} 기간의 ${getRankingTypeLabel(rankingType)} 랭킹이 없습니다.` 
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">소설</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">채팅 수</th>
              </tr>
            </thead>
